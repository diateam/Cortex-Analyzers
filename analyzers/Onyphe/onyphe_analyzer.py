    ...
    def run(self):
        ...
        try:
            ...
            if self.service == 'threats':
                ...
                self.report(results)
            if self.service == 'ports':
                """Return synscan information we have for the given IPv{4,6} address with history of changes
                """
                ip = self.getParam('data', None, 'Data is missing')
            ...
            if self.service == 'geolocate':
                ...
                self.report(results)
            if self.service == 'reverse':
                """Return reverse DNS lookup information we have for the given IPv{4,6} address with history of changes
                """
                ip = self.getParam('data', None, 'Data is missing')
